...
const cardElement = (city) => `<div id="card" data-id="${city.id}">
                ...
                <img class="icon" src="${"http://openweathermap.org/img/wn/" + city.icon + "@2x.png"}" alt="" />
                <div class="main">${city.main}</div>
                <button class="delete" href="#" onclick="deleteCity(${city.id})">Delete</button>
                    <ion-icon name="trash-outline"></ion-icon
                >
            </div>`


const checkCityExist = (data) => cityList.filter((city) => {
    ...
    await fetch(
        ...
        .then((data) => {
            if (checkCityExist(data)) {
                ...
                document.querySelector(".cardwrapper").insertAdjacentHTML("beforeend", card);
            } else {
                Toastify({
